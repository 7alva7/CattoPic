...
- 详情弹层改为左右布局：左侧预览图，右侧元数据与复制链接（窄屏仍上下排列）。
- 弹窗遮罩/面板、主按钮/次按钮/危险按钮、浮动按钮统一为同一套样式。
- 详情弹层铺满可视区域：左侧大图、右侧紧凑链接列表。API Key / 标签 / 随机图弹窗按内容定高。操作按钮统一 40px 高。
- 前端交互整理：弹窗按内容收紧或铺满工作区；操作按钮、输入框、分段控件统一 40px 高；嵌套弹窗挂到 `document.body`，避免被裁切。API 密钥改为单一表单，不再出现几乎空白的「管理」弹层。标签重命名改为行内编辑。随机图 API 标签改为同一组三态芯片。压缩设置收入上传卡片。
