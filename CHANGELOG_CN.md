...
- 详情弹层改为固定尺寸左右布局：左侧预览用 `object-cover` 铺满，右侧为元数据与复制链接。操作按钮与 API 密钥弹窗同一套实心 indigo / 描边次按钮。正文字体为 Inter + Noto Sans SC 500，字色加深。
- 弹窗统一为卡片语言：`rounded-2xl` 面板、圆形图标徽标、与 API 密钥相同的标题栏。标签删除改为左对齐标题，不再居中放大警告图标。嵌套的编辑/删除确认挂到页面上，避免被裁切。
