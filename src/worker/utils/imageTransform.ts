...
export function buildImageUrls(params: {
  ...
  const avifUrl = (() => {
    ...
    const avifCap = 1200;
    const avifMaxW = hasResizeLimit ? Math.min(maxWidth, avifCap) : avifCap;
    const avifMaxH = hasResizeLimit ? Math.min(maxHeight, avifCap) : avifCap;
    const dims = calculateDimensions(image.width, image.height, avifMaxW, avifMaxH);
