...
export class CompressionService {
  ...
  async compress(
    ...
    // AVIF long side is capped at 1200px by Cloudflare Images limits
    const avifMaxWidth = opts.maxWidth > 0 ? Math.min(opts.maxWidth, 1200) : 1200;
    const avifMaxHeight = opts.maxHeight > 0 ? Math.min(opts.maxHeight, 1200) : 1200;
